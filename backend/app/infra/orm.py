...
class SeriesRow(Base):
    ...
    instances: Mapped[list[InstanceRow]] = relationship(
        ...
        lazy="selectin",
        order_by="InstanceRow.slice_index",
    )
...
class InstanceRow(Base):
    ...
    slice_index: Mapped[int] = mapped_column(Integer, default=0, index=True)
    slice_position: Mapped[float | None] = mapped_column(Float, nullable=True)
